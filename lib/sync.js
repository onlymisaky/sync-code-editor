...
const { getEditorPath, getEditorList, getSyncItemPath, getSyncItemInfo } = require('./editors');
 ...
 */
async function syncConfigs(sourceEditor, syncItems) {
  const results = [];
  const sourceEditorPath = getEditorPath(sourceEditor);
  const targetEditors = getEditorList().filter(editor => editor.value !== sourceEditor);
