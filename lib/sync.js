...
const { getEditorInfo, getConfigurationFiles, getConfigurationInfo, getConfigurationFileFullPath } = require('./editors');
...
async function syncConfig(sourceEditorKey, configurationKey, targetEditorKey) {
  const configurationFiles = getConfigurationFiles(sourceEditorKey, configurationKey);

  const missingFiles = [];
  const errorFiles = [];

  let promises = configurationFiles.map((config) => {
  ...
  const tasks = configurationFiles.map((config, index) => {
    return {
      type: config.type,
      from: config.path,
...
function printSyncResults(syncResult) {
  ...
  Object.entries(syncResult).map(([targetEditorName, results]) => {
    ...
    results.forEach((result) => {
      if (result.success) {
        console.log(`   ✓ ${result.configurationName}`);
        successCount++;
      } else if (!result.success) {
        console.log(`   ✗ ${result.configurationName}: ${result.message}`);
        errorCount++;
      }
      else {
        // TODO
